...
- `scripts/render_mixed.py` — 深色容器截图 + 正文条目保留 HTML 的混合渲染
...
公众号编辑器对深色背景、渐变、flex 布局支持有限，容易样式丢失。
因此将深色容器（报头区）用 Playwright 渲染为截图，正文条目保留 HTML。

**原理**：深色容器（组件 2）→ 截图 `.png`，正文条目（组件 3+4）+ CTA（组件 5）→ 保持 HTML

```bash
python3 "$SKILL_ROOT/scripts/render_mixed.py" <装配好的.html>
```

产出：
| 文件 | 说明 |
|------|------|
| `{原文件}_混合.html` | 深色容器替换为 `<img>` 的最终 HTML |
| `{原文件}_header.png` | 深色容器的 2× 高清截图 |

**使用注意**：
- 截图文件（`_header.png`）需要和 HTML 放在同一目录
- 粘贴到公众号后，将 `<img>` 的 `src` 替换为微信素材库的图片链接
...
**产物格式**：最终交付的是混合渲染后的文件（`{...}_混合.html`），深色容器为图片，正文为纯 `<section>…</section>` 片段。

1. **混合正文文件**：`{原文件名}_排版_AI 早报 (ai-daily)_混合.html`（共生效的 HTML）
2. **报头截图**：`{...}_header.png`（与 HTML 同一目录，需上传微信素材库）
3. **带「复制」按钮的预览页**：
   ```bash
   <SKILL_ROOT>/scripts/wrap_preview.py <上面的混合.html>
   ```
...
4. 告知用户：
   ...
   ✅ AI 早报已生成 →
     混合正文：{路径}_混合.html
     报头截图：{路径}_header.png（请上传至微信素材库并替换 src）
     预览页：  {路径}_预览.html
     外标题：  AI 早报｜{要点新闻}
   ```
