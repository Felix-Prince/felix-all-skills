...
把一份 JSON 格式的 AI 资讯数据转换为可直接复制粘贴进微信公众号编辑器、且粘贴后样式不丢失的 HTML。

核心资产是 `references/` 下的**主题组件库**（`theme-ai-daily.md`：设计变量 + 各组件完整 HTML + 模板骨架 + 映射规则）外加 `references/data-schema.md` 定义的**标准数据结构**。本 SKILL.md 只负责流程与决策，**具体 HTML 代码一律从组件库取，不要凭记忆手写**。

## 工作流

### 0. 输入与数据校验

用户给 JSON 数据（`.json` 文件或粘贴的 JSON 文本）。先读 `references/data-schema.md` 校验字段：

**必填字段**：`items[].title`、`items[].summary`、`items[].source`、`items[].category`

**可选字段**：`items[].id`、`items[].sourceUrl`、`items[].publishedAt`、`items[].relativeTime`、`items[].tags`、`items[].importance`、`items[].featured`

**category 取值**：`domestic`（国内）/ `international`（国际），其它值报错。

字段缺失时向用户索要或自动补全（`importance` 默认 5，`featured` 默认 false）。

用户说「直接排 / 自动排 / 一键」时进**全自动模式**：跳过结构确认，自动排序、选速览、排版校验，交付时附决策说明。
...
它确定性地检查平台禁用项和 `<span leaf>` 包裹。报 ERROR 就回到第 3 步修；**半角标点 WARNING 同样要修复到 0 再交付**。
...
- **漏 `<span leaf>` 包裹**是最常见致命错——粘贴到公众号后样式整片丢失。靠第 4 步校验脚本兜底，别跳过
