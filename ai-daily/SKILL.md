...
- `scripts/render_mixed.py` — 将装配好的 HTML 渲染为整篇长图
...
公众号对深色背景、渐变、flex 布局支持有限，直接将 HTML 粘贴容易样式丢失。
因此整篇文章用 Playwright 渲染为一张高清大图（2× Retina），样式 100% 保真。
...
# 将装配好的 HTML 渲染为长图
python3 "$SKILL_ROOT/scripts/render_mixed.py" <装配好的.html> --output <输出.png>
```

**注意**：
- 生成前保证装配好的 HTML 已通过校验脚本检查（标点、span leaf 等）

---

### 5. 校验合规（强制）

把生成的 HTML 写入临时文件后，**必须运行校验脚本**，ERROR 清零才可进入渲染步骤：

```bash
<SKILL_ROOT>/scripts/validate_gzh_html.py <装配好的.html>
```
...
**产物格式**：最终交付的是一张长图，可直接上传微信公众号文章。

1. **长图文件**：`{YYYYMMDD}_AI早报.png`（工作目录）
2. 告知用户：
   ```text
   ✅ AI 早报已生成 →
     长图：    {文件名}.png
